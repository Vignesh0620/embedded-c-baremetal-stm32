...
/* USER CODE BEGIN PFP */
void  Delay_ms(uint32_t ms)     { uint32_t s = HAL_GetTick(); while((HAL_GetTick() - s) < ms);}
/* USER CODE END PFP */
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  {
		  SPI1_CS_Low();
		  uint8_t rx = SPI1_Transfer(tx[i]);
		  SPI1_CS_High();
		  if(rx == tx[i]);
		  	  all_pass=1;

		  	 /*Blink LED Once for each successfult byte*/
		  GPIOB->BSRR = (1u<<2);
		  Delay_ms(200);
	  ...
	  if(all_pass)
		  Delay_ms(1000); //Pause after 4 blinks
	  else
	  {
		  /* Error indication */
		         while(1)
